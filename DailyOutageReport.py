...
# Function to calculate Total Allowable Limit (Hr)
def calculate_allowable_limit(total_site_count):
    if pd.notnull(total_site_count):
        total_hours = total_site_count * 24 * 30
        allowable_limit = total_hours - (total_hours * 0.9985)
        return Decimal(allowable_limit).quantize(Decimal('0.00'), rounding=ROUND_HALF_UP)
    return Decimal(0.0)

# Function to calculate Remaining Hour
def calculate_remaining_hour(total_allowable_limit, elapsed_time):
    return total_allowable_limit - elapsed_time
...
if alarm_history_file:
    ...
    try:
        ...
        for tenant in df_alarm_history["Tenant"].unique():
            ...
            # Add Total Allowable Limit and Remaining Hour
            merged_data["Total Allowable Limit (Hr)"] = merged_data["Total Site Count"].apply(calculate_allowable_limit)
            merged_data["Remaining Hour"] = merged_data.apply(
                lambda row: calculate_remaining_hour(row["Total Allowable Limit (Hr)"], row["Elapsed Time (Decimal)"]),
                axis=1
            )
