...
# Define the path to the MTA Site List file
base_dir = os.path.dirname(os.path.abspath(__file__))  # Directory where the script is located
mta_site_file_path = os.path.join(base_dir, "MTA_Site_List.xlsx")  # Adjust file name/path as needed

# Check if the file exists and load it
if os.path.exists(mta_site_file_path):
    try:
        # Load the MTA Site List file
        df_mta_site = pd.read_excel(mta_site_file_path, skiprows=2)
        df_mta_site = df_mta_site[~df_mta_site["Site Alias"].str.startswith("L", na=False)]  # Example filtering
        print("MTA Site List loaded successfully!")
    except Exception as e:
        print(f"Error processing MTA Site List: {e}")
else:
    print(f"MTA Site List file not found at: {mta_site_file_path}")
